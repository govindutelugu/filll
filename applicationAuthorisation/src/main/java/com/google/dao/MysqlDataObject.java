...
public class MysqlDataObject {
	public static PrivilegeVo getUserRoles(String userId) {
		...
		try{  
			...
			Statement privilegestmtObj=dbConnectionObj.createStatement();
			String sqlQuery = "select description from privilege_group;";
			ResultSet privilegeRs= privilegestmtObj.executeQuery(sqlQuery);
			
			String preparedstmtQuery ="select pg.description,up.role from USER_PRIVILEGES up,privilege_group pg where user_id=(select user_id from user where name=?)\n"
					+ "and pg.privilege_id=up.privilege_group_id;";
			PreparedStatement userRoleStmtObj=dbConnectionObj.prepareStatement(preparedstmtQuery); 
			
			userRoleStmtObj.setString(1,userId);
			ResultSet userRoleRs=userRoleStmtObj.executeQuery(); 
			while(privilegeRs.next()) {
				privilegeList.add(privilegeRs.getString(1));
			}
			
			System.out.println("Priting array values:"+privilegeList);
			while(userRoleRs.next()) { 
				...
				userRoleObj.setPrivileges(userRoleRs.getString(1)+"_"+userRoleRs.getString(2));
				rolesObj.add(userRoleObj);
			}
			
			if(dbConnectionObj!=null){dbConnectionObj.close();}
			
			privilegeObj.setApps(privilegeList);
